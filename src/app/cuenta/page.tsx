...
import { clienteDemo, cuponesDemo, mascotasDemo, pedidosDemo } from "@/data/cuenta";
import { reglaPuntos, siguienteRecompensa } from "@/data/recompensas";
...
export default function ResumenCuenta() {
  ...
  const cuponesActivos = cuponesDemo.filter((c) => !c.usado);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-6">
          <div>
            ...
            <p className="mt-1 text-sm text-petroleo-100">
              Miembro desde {fechaCorta(clienteDemo.desde)}
            </p>
            ...
            {siguiente ? (
              <div className="mt-5 max-w-sm">
                ...
                <p className="mt-2 text-sm text-petroleo-100">
                  ...
                  {siguiente.nombre.toLowerCase()} · {precio(faltan * reglaPuntos.montoPorPunto)}{" "}
                  en compras
                </p>
              </div>
            ) : null}
        ...
        {[
          {
            href: "/cuenta/pedidos",
            icono: Package,
            valor: pedidosDemo.length,
            texto: "pedidos realizados",
          },
          {
            href: "/cuenta/mascotas",
            icono: Dog,
            valor: mascotasDemo.length,
            texto: "mascotas registradas",
          },
          {
            href: "/cuenta/cupones",
            icono: Ticket,
            valor: cuponesActivos.length,
      ...
      <section>
        ...
        <TarjetaPedido pedido={pedidosDemo[0]} />
      </section>
        ...
        <ul className="mt-5 grid gap-3 sm:grid-cols-2">
          {cuponesActivos.map((c) => (
            ...
            >
              ...
              <p className="mt-1 text-xs text-grafito">Vence el {fechaCorta(c.vence)}</p>
            </li>
          ))}
        </ul>
      </section>
