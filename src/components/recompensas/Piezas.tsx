...
import type { EstadoPuntos, MovimientoPuntos, Recompensa } from "@/lib/tipos";
import { nombreEstadoPuntos } from "@/data/recompensas";
import { canjearRecompensa } from "@/server/acciones/recompensas";
...
export function CatalogoRecompensas({
  recompensas,
  puntos,
  autenticado,
}: {
  ...
  const [estado, accion] = useActionState(canjearRecompensa, ESTADO_INICIAL);
        ...
        {recompensas.map((r) => {
          ...
          return (
            ...
            >
              ...
              {!autenticado ? (
                <Boton href="/ingresar" variante="contorno" medida="md" className="mt-5 w-full">
                  Inicia sesión para canjear
                </Boton>
              ) : alcanzable ? (
                <form action={accion} className="mt-5">
                  <input type="hidden" name="recompensaId" value={r.id} />
                  <BotonEnviar medida="md" className="w-full" enviando="Canjeando…">
...
export function HistorialPuntos({
  ...
  const lista =
    filtro === "todos"
      ? movimientos
      : movimientos.filter((m) => m.estado === filtro);
      ...
      <div className="sin-scrollbar flex gap-2 overflow-x-auto border-b border-petroleo-700/10 p-4">
        {FILTROS.map((f) => {
          const cantidad =
            f.id === "todos"
              ? movimientos.length
              : movimientos.filter((m) => m.estado === f.id).length;
          return (
            <button
              key={f.id}
              type="button"
      ...
      {lista.length === 0 ? (
        <p className="p-8 text-center text-sm text-grafito">
          {movimientos.length === 0
            ? "Todavía no tienes movimientos. Tu primer pedido empieza a sumar."
